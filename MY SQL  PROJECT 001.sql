...
SELECT count(personName) FROM billionaires
WHERE gender="M" AND finalWorth>30000 AND country IN("CANADA" ,"UNITED STATES","CHINA");

SELECT firstName, lastName,finalWorth,tax_revenue_country_country,country  FROM billionaires
WHERE finalWorth BETWEEN 30000 AND 70000
ORDER BY country DESC,finalWorth DESC
# THE CODE BELOW RETURNS THE NEXT 12 ROWS  FROM 8
LIMIT 8,12;
...
SELECT count(personName) FROM billionaires
WHERE   AGE > 50 AND category = "TECHNOLOGY";
